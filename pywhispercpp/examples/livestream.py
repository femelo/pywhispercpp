...
class LiveStream:
    ...
    def _audio_callback(self, outdata, frames, time, status):
        ...
        if self.audio_data.size > self.samplerate:
            ...
            p1 = Process(
                target=self._transcribe_process,
            )
            p1.start()
            self.audio_data = np.array([])
...
def _main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="block size, default to %(default)s",
    )
    parser.add_argument(
        "-bus",
        "--buffer_size",
        type=int,
        default=20,
        help="number of blocks used for buffering, default to %(default)s",
    )
    parser.add_argument(
        "-ss",
        "--sample_size",
        type=int,
        default=4,
        help="Sample size, default to %(default)s",
    )
